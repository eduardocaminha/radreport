---
tipo: tc-coluna-cervical
contraste: sem
urgencia_padrao: true
---

TOMOGRAFIA COMPUTADORIZADA DA COLUNA CERVICAL
<!-- OPCIONAL:urgencia - remover se "eletivo", "ambulatorial" ou similar -->
Exame realizado em caráter de urgência
<!-- /OPCIONAL:urgencia -->

TÉCNICA:
Aquisição volumétrica multislice, sem a administração endovenosa de meio de contraste iodado.

ANÁLISE:
<!-- REGIAO:transicao_craniovertebral -->
Transição craniovertebral sem alterações.
<!-- /REGIAO:transicao_craniovertebral -->

<!-- REGIAO:processo_odontoide -->
Processo odontoide centrado, com contornos regulares.
<!-- /REGIAO:processo_odontoide -->

<!-- REGIAO:corpos_vertebrais -->
Corpos vertebrais alinhados e com alturas preservadas.
<!-- /REGIAO:corpos_vertebrais -->

<!-- REGIAO:pediculos_arco_posterior -->
Pedículos e demais estruturas do arco posterior sem anormalidades.
<!-- /REGIAO:pediculos_arco_posterior -->

<!-- REGIAO:articulacoes -->
Articulações interapofisárias e uncovertebrais de contornos regulares.
<!-- /REGIAO:articulacoes -->

<!-- REGIAO:nivel_c2_c3 -->
<!-- /REGIAO:nivel_c2_c3 -->

<!-- REGIAO:nivel_c3_c4 -->
<!-- /REGIAO:nivel_c3_c4 -->

<!-- REGIAO:nivel_c4_c5 -->
<!-- /REGIAO:nivel_c4_c5 -->

<!-- REGIAO:nivel_c5_c6 -->
<!-- /REGIAO:nivel_c5_c6 -->

<!-- REGIAO:nivel_c6_c7 -->
<!-- /REGIAO:nivel_c6_c7 -->

<!-- REGIAO:nivel_c7_t1 -->
<!-- /REGIAO:nivel_c7_t1 -->

<!-- REGIAO:discos_intervertebrais -->
Discos intervertebrais com alturas preservadas, sem protrusões significativas.
<!-- /REGIAO:discos_intervertebrais -->

<!-- REGIAO:canal_vertebral_forames -->
Diâmetros normais do canal vertebral e dos forames intervertebrais.
<!-- /REGIAO:canal_vertebral_forames -->

<!-- REGIAO:planos_musculares -->
Planos musculares paravertebrais sem alterações significativas ao método.
<!-- /REGIAO:planos_musculares -->
